...
function myPow(x, n) {
    ...
    if (n === 1)
        return x;
    if (n < 0)
        return 1 / callRecFunction(x, Math.abs(n));
    else
        return callRecFunction(x, Math.abs(n));
}
function callRecFunction(x, n) {
    if (n === 0) {
        return 1;
    }
    if (n % 2 === 0) {
        return Math.pow(callRecFunction(x, n / 2), 2);
    }
    else {
    }
    return Math.pow(callRecFunction(x, (n - 1) / 2), 2) * x;
}
/**
 * Example 1:
...
function helper3(arr, sum, tempSum, i) {
    ...
    if (i >= arr.length)
        return false;
    return (helper3(arr, sum, tempSum + arr[i], i + 1) ||
